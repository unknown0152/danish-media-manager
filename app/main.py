...
def search(
    ...
    for release in releases:
        SEARCH_CACHE[release.result_id] = {
            "download_url": release.download_url,
            "title": release.title,
            "media_type": request.media_type,
        }
    return SearchResponse(query=request.query, media_type=request.media_type, releases=releases)
...
def grab(
    ...
    if request.result_id and not request.download_url:
        cached = SEARCH_CACHE.get(request.result_id)
        if not cached:
            raise HTTPException(status_code=404, detail="Search result expired; search again")
        request.download_url = cached.get("download_url") if isinstance(cached.get("download_url"), str) else None
